...
def test_mask_variables_debug_no_params():
    ...
    assert f"'service1_password': '{MASKED_VARIABLE}'" in result.stdout
    assert f"'service2_token': '{MASKED_VARIABLE}'" in result.stdout
...
@pytest.mark.e2e
def test_mask_variables_print_event():
    """
    Validate that when a global debug action is used, that the necessary
    variables are masked
    """

    rulebook = utils.BASE_DATA_PATH / "rulebooks/test_print_event.yml"
    vars_file = utils.BASE_DATA_PATH / "extra_vars/test_debug.yml"
    inventory = utils.BASE_DATA_PATH / "inventories/default_inventory.ini"
    cmd = utils.Command(
        rulebook=rulebook,
