...
async def test_84_debug_mask_sensitive_variables(caplog):
    ...
    with SourceTask(rs.sources[0], "sources", {}, queue):
        ...
        assert f"'controller_password': '{MASKED_VARIABLE}'" in caplog.text
        assert f"'tower_password': '{MASKED_VARIABLE}'" in caplog.text
        assert f"'aap_password': '{MASKED_VARIABLE}'" in caplog.text
        assert f"'postgres_db_password': '{MASKED_VARIABLE}'" in caplog.text
